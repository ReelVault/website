...
interface SidebarCSSProperties extends CSSProperties {
...
function SidebarProvider({
	...
	const setOpen = useCallback(
		(next: boolean | ((current: boolean) => boolean)) => {
			const openState = typeof next === "function" ? next(open) : next;
			if (setOpenProp) {
			...
			<Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
				...
				>
					<SheetHeader className="sr-only">
						<SheetTitle>{m.common_sidebar()}</SheetTitle>
						<SheetDescription>{m.common_mobile_sidebar_description()}</SheetDescription>
					</SheetHeader>
...
function SidebarTrigger({ className, onClick, ...props }: ComponentProps<typeof Button>) {
	...
	const handleClick = (event: Parameters<NonNullable<ComponentProps<typeof Button>["onClick"]>>[0]) => {
		onClick?.(event);
		toggleSidebar();
	};
...
function SidebarRail({ className, ...props }: ComponentProps<"button">) {
	...
	return (
		<button
			...
			className={cn(
				"absolute inset-y-0 z-20 hidden w-4 transition-[background-color,transform] ease-linear after:absolute after:inset-s-1/2 after:inset-y-0 after:w-0.5 hover:after:bg-sidebar-border group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2",
