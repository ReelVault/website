import { Calendar, Film, RefreshCw } from "lucide-react";
...
interface IdentifyCandidateCardProps {
	candidate: ProviderCandidate;
	isCurrentSelecting: boolean;
	isApplying: boolean;
	actionMode: "replace" | "supplement";
	onApply: (providerId: string, externalId: string) => void;
}

export function IdentifyCandidateCard({ candidate, isCurrentSelecting, isApplying, actionMode, onApply }: IdentifyCandidateCardProps) {
	const releaseYear = candidate.releaseDate ? candidate.releaseDate.slice(0, 4) : null;
	const applyLabel = actionMode === "supplement" ? m.components_identify_fill_gaps() : m.components_apply_match();
